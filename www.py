...
def load_model():
    """Carga el modelo YOLO11 para detección de vehículos"""
    try:
        # Importar dinámicamente para evitar problemas de análisis estático
        _YOLO = None
        try:
            ul = importlib.import_module('ultralytics')
            _YOLO = getattr(ul, 'YOLO', None)
        except Exception:
            _YOLO = None
        if _YOLO is None:
            _YOLO = getattr(importlib.import_module('ultralytics.yolo.engine.model'), 'YOLO')
        print("Cargando modelo YOLO11 Medium (más preciso)...")
        # Usar yolo11m para mejor precisión (balance entre velocidad y precisión)
        model = _YOLO('yolo11m.pt')  # yolo11n < yolo11s < yolo11m < yolo11l < yolo11x
        
        # Configurar parámetros para mejor precisión
        model.overrides['conf'] = 0.3  # Threshold de confianza
        model.overrides['iou'] = 0.5   # IoU threshold para NMS
        model.overrides['max_det'] = 100  # Máximo de detecciones por imagen
        
        print("Modelo YOLO11 Medium cargado exitosamente")
        print(f"Tipo de modelo: {type(model)}")
        print(f"Configuración: conf={model.overrides['conf']}, iou={model.overrides['iou']}")
        return model
    except Exception as e:
        print(f"Error al cargar el modelo: {e}")
        print("Intentando descargar el modelo...")
        # Si falla, intenta descargar
        ul = importlib.import_module('ultralytics')
        _YOLO = getattr(ul, 'YOLO', None)
        if _YOLO is None:
            _YOLO = getattr(importlib.import_module('ultralytics.yolo.engine.model'), 'YOLO')
        model = _YOLO('yolo11m.pt')
        return model

def get_bboxes(preds):
    """Extrae bounding boxes de las predicciones de YOLO11 con tipos de vehículos y filtros mejorados"""
    detections = []
    ...
    # YOLO11 devuelve una lista de resultados
    if len(preds) > 0:
...
def detector(cap):
    ...
    while cap.isOpened():
        ...
        for detection in bboxes:
            ...
            # Centro en coordenadas del frame RAW ORIGINAL (donde se hizo la detección)
            xc, yc = get_center(box)
            matched = False
            
            # Buscar el vehículo más cercano en el tracking
            ...
            for car_id, car_info in tracked_cars.items():
                ...
                distance = np.sqrt((xc - prev_x)**2 + (yc - prev_y)**2)
                
                # Verificar que sea el mismo tipo de vehículo (más robusto)
                prev_type = car_info.get('type', vehicle_type)
                
                # Threshold adaptativo basado en el tamaño del vehículo
                max_distance = max(50, (width + height) / 4)
                
                # Si está dentro del threshold y es el más cercano
                if distance < max_distance and distance < min_distance:
                    # Permitir cambio de tipo solo si es similar (ej: auto<->camion)
                    if prev_type == vehicle_type or \
                       (prev_type in ['auto', 'camion'] and vehicle_type in ['auto', 'camion']):
                        min_distance = distance
                        best_match_id = car_id

            if best_match_id is not None:
                car_info = tracked_cars[best_match_id]
                ...
                current_positions[best_match_id] = {
                    'pos': (xc, yc),
                    'type': vehicle_type,
                    'conf': conf,
                    'frames_tracked': car_info.get('frames_tracked', 0) + 1,
            ...
            if not matched:
                # Nuevo vehículo detectado - iniciar tracking
                in_lanes = {}
                for lane_name, lane_zone in lanes_count.items():
                    is_in_lane = is_valid_detection(xc, yc, lane_zone)
                    in_lanes[lane_name] = is_in_lane
                    in_lanes[f'counted_{lane_name}'] = False  # No contar inmediatamente
                
                # Crear nuevo vehículo en tracking (requiere confirmación en frames siguientes)
                current_positions[next_id] = {
                    'pos': (xc, yc),
                    'type': vehicle_type,
                    'conf': conf,
                    'frames_tracked': 1,
                    'confirmed': False,  # Requiere estar en múltiples frames
                    **in_lanes
                }
                next_id += 1
            ...
            # Etiqueta del tipo de vehículo con confianza
            label = f"{vehicle_type} {conf:.2f}"
            cv2.putText(frame, label, (box_disp[0], box_disp[1] - 10),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2, cv2.LINE_AA)

        # Limpiar vehículos que llevan mucho tiempo sin ser detectados
        # Mantener solo vehículos que fueron vistos recientemente
        tracked_cars = {}
        for car_id, car_info in current_positions.items():
            # Confirmar vehículos que han sido rastreados por al menos 2 frames (mejor recall)
            if car_info.get('frames_tracked', 0) >= 2:
                car_info['confirmed'] = True
        ...
        model_info = "YOLO11-Medium | Conf: 0.5+ | IoU: 0.5"
